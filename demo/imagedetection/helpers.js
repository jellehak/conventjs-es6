...
export var sample_training_instance = function () {
...
export var sample_test_instance = function () {
  var b = test_batch
...
export const load_data_batch = function (batchNum, path = 'input') {
...
export var draw_activations = function (elt, A, scale, grads) {
  var s = scale || 2 // scale
...
export var draw_activations_COLOR = function (elt, A, scale, grads) {
  var s = scale || 2 // scale
  var draw_grads = false
  if (typeof (grads) !== 'undefined') draw_grads = grads
...
export var visualize_activations = function (net, elt) {
  // clear the element
  elt.innerHTML = ''
  ...
  for (var i = 0; i < N; i++) {
    ...
    if (L.layer_type === 'conv') {
      ...
      if (L.filters[0].sx > 3) {
        ...
        filters_div.appendChild(document.createElement('br'))
        for (var j = 0; j < L.filters.length; j++) {
          if (i === 1) { draw_activations_COLOR(filters_div, L.filters[j], 2, true) } else {
            filters_div.appendChild(document.createTextNode('('))
...
export var load_and_step = function () {
  if (paused) return
...
export var testImage = function (img) {
...
export const step = function (sample) {
  var x = sample.x
  var y = sample.label
